...
Alps Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9667 for connections and services them.
...
However, in open source software such as Alps Core, where everyone uses
...
pay attention to for reviewers of Alps Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Alps Core uses tinyformat, which is type safe. Leave them out to avoid confusion
