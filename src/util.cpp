...
const char * const BITCOIN_CONF_FILENAME = "alps.conf";
const char * const BITCOIN_PID_FILENAME = "alpsd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "alps" is a composite category enabling all Alps-related debug output
            if(ptrCategory->count(string("alps"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AlpsCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AlpsCore
    // Mac: ~/Library/Application Support/AlpsCore
    // Unix: ~/.alpscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AlpsCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AlpsCore";
#else
    // Unix
    return pathRet / ".alpscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty alps.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override alps.conf
