...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "AlpsNetwork") type = AlpsNetwork;
    if(strType == "AlpsNetworkVariable") type = AlpsNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == AlpsNetwork) s = "AlpsNetwork";
    if(type == AlpsNetworkVariable) s = "AlpsNetworkVariable";
